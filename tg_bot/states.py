...
def text_locator_state(state_class):
    ...
    class WrappedStateClass(state_class):
        def process(self, event):
            ...
            SendMessageRequest(
                # text=f'currently state: {state.state_class_locator}',
...
@router.register('/text-state/')
class TextState(PrivateChatState):
    def enter_state(self, message) -> Locator | None:
        SendMessageRequest(
            text=message,
            chat_id=Conversation.current.tg_chat_id,
        ).send()
...
@router.register('/four-menu/')
class FourButtonState(PrivateChatState):

    def enter_state(self) -> Locator | None:
        mailing_text = dedent('''\
            *📨 1️⃣ It's a four buttons!*
